...
Page({
  ...
  addOrder: function () {
    ...
    wx.request({
      ...
      data: {
        ...
        good_id: 1,
        name: '1',
        cover: 'https://img.alicdn.com/imgextra/i2/1058052659/TB2vFuii4HI8KJjy1zbXXaxdpXa_!!1058052659-0-beehive-scenes.jpg_300x300.jpg',
        token: that.data.token,
      },
      header: {
        "Content-Type": "application/x-www-form-urlencoded",
  ...
  lastPage: function (toPageNo) {
    ...
    wx.request({
      url: 'https://passion.njshangka.com/easywin/m/e/myfavorite',
      method: "POST",
      data: {
        "mall_id": that.data.mall_id,
        page_no: toPageNo,
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          var newSchemefootlistArr = ordersfootlistArr.concat(res.data.data.favorites)
          if (res.data.data.favorites.length == 0) {
            that.setData({
            ...
            console.log(res.data.data.favorites.length)
          } else {
            console.log(res.data.data.favorites.length)
            that.setData({
