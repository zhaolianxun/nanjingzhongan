...
Page({
  ...
  lastPage: function (toPageNo) {
    ...
    wx.request({
      url: 'https://passion.njshangka.com/easywin/m/e/mlc/withdrawrecord/ent',
      method: "POST",
      data: {
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          for (var i = 0; i < that.data.list.length; i++) {
            applyTime = that.data.list[i].applyTime
            that.data.list[i].applyTime = that.dateChange(applyTime)
          }
