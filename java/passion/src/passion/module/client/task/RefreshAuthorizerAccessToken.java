...
public class RefreshAuthorizerAccessToken {
	...
	public void run() {
		...
		try {
			...
			connection = PassionDataSource.dataSource.getConnection();
			pst = connection.prepareStatement(
					"select id,refresh_token,app_id  from t_wx_miniapp where authorized=1 and ((authorization_time + expires_in*1000) - rpad(REPLACE(unix_timestamp(now(3)),'.',''),13,'0')) <= 600  limit 0,50");
			...
			while (rs.next()) {
				...
				jo.put("appId", rs.getString("app_id"));
				ja.add(jo);
			}
			pst.close();
			...
			for (int i = 0; i < ja.size(); i++) {
				try {
					...
					pst = connection.prepareStatement("select count(id) from t_wx_miniapp where id=? for update");
					pst.setString(1, id);
					rs = pst.executeQuery();
					if (!rs.next())
						throw new InteractException("记录不存在");
					pst.close();
					...
					pst = connection.prepareStatement(
							"update t_wx_miniapp set access_token=?,expires_in=?,refresh_token=? where id=?");
					...
					if (n != 1)
						throw new InteractException("更新失败");
					pst.close();
					connection.commit();
				} catch (Exception e) {
