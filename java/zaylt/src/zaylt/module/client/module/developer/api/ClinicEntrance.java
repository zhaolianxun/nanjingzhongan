...
public class ClinicEntrance {
	...
	public void patients(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 查詢订单列表
			pst = connection.prepareStatement(
					"select t.id,t.realname,t.tel,u.name clinic_name from t_patient t left join t_clinic u on t.clinic_id=u.id where t.clinic_id=?  order by t.realname asc limit ?,?");
			pst.setObject(1, clinicId);
