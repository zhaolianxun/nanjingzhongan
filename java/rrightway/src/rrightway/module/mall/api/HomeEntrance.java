...
public class HomeEntrance {
	...
	public void goodDetailEntrance(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 查询优惠券
			pst = connection.prepareStatement(
					"select t.id,t.title,t.desc,t.type,t.type1_uptomoney,t.type1_submoney,t.type1_starttime,t.type1_endtime,IF(tt.id is null, 0, 1) getted,IF(tt.used is null, 0, tt.used) used from t_mall_coupon t left join t_mall_usercoupon tt on t.id=tt.coupon_id and tt.user_id=? where t.mall_id=? order by t.add_time desc limit 0,30");
			if (loginStatus != null)
	...
	public void getcoupon(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			pst = connection.prepareStatement(
					"select t.type,t.type1_uptomoney,t.type1_submoney,t.type1_starttime,t.type1_endtime,IF(tt.id is null, 0, 1) getted from t_mall_coupon t left join t_mall_usercoupon tt on t.id=tt.coupon_id where t.id=? and tt.user_id=?");
