...
public class UserAction {
	...
	public void login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (userId == null) {
				...
				pst = connection.prepareStatement("update t_mall_user set wx_openid=?,wx_sessionkey=? where id=?");
				pst.setObject(1, openid);
			...
			if (oldToken != null)
				jedis.del(new StringBuilder("rrightway.mall.login-").append(oldToken).toString());
			jedis.del(userId);
			//// 设置新登录状态
	...
	public void selfInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String loginRedisKey = new StringBuilder("rrightway.mall.login-").append(loginStatus.getToken()).toString();
			jedis.set(loginRedisKey, JSON.toJSONString(loginStatus));
			jedis.expire(loginRedisKey, 30 * 24 * 60 * 60);
	...
	public static void main(String[] args) {
		...
		String loginRedisKey = new StringBuilder("rrightway.mall.login-").append("999999999999").toString();
		jedis.set(loginRedisKey, JSON.toJSONString(loginStatus));
		jedis.close();
	}
}